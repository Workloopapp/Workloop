-- Let public booking clients ask for suggestions on one customer-selected
-- date while preserving the existing capped, privacy-safe default window.

create or replace function app_private.get_public_booking_slot_suggestions_core(
  p_handle text,
  p_service_id uuid,
  p_source_hash text,
...
begin
  ...
  if p_target_date is not null
     and (
       p_target_date < v_local_today
       or p_target_date > v_local_today + (v_window_weeks * 7)
     ) then
    v_first_date := v_local_today + 1;
    v_last_date := v_local_today;
  else
    v_first_date := coalesce(p_target_date, v_local_today);
    v_last_date := case
      when p_target_date is not null then p_target_date
      else least(
        v_local_today + 27,
...
revoke all on function app_private.get_public_booking_slot_suggestions_core(
  text, uuid, text, integer, date
) from public, anon, authenticated;
grant execute on function app_private.get_public_booking_slot_suggestions_core(
  text, uuid, text, integer, date
) to service_role;
...
set search_path = ''
as $$
  select app_private.get_public_booking_slot_suggestions_core(
    p_handle,
    p_service_id,
    p_source_hash,
    p_extra_duration,
...
revoke all on function public.get_public_booking_slot_suggestions_v3(
  text, uuid, text, uuid[], date
) from public, anon, authenticated;
grant execute on function public.get_public_booking_slot_suggestions_v3(
  text, uuid, text, uuid[], date
) to service_role;

comment on function public.get_public_booking_slot_suggestions_v3(
  text, uuid, text, uuid[], date
) is
  'Service-role-only public booking suggestions for the default capped window or one customer-selected date. Returns UTC starts without diary metadata.';
