-- A deposit is a requested part of one invoice, never an extra payment row.
-- Only actual receipt/refund deltas affect cash totals. Issued terms are fixed.
alter table public.business_documents
  ...
  add column deposit_value numeric not null default 0,
  add column deposit_amount numeric(14,2) not null default 0,
  add column deposit_due_date date,
  add constraint business_documents_deposit_check check (
    (deposit_type='none' and deposit_value=0 and deposit_amount=0 and deposit_due_date is null)
    or (deposit_type in ('fixed','percentage') and deposit_value>0 and deposit_value=round(deposit_value,2)
      and deposit_amount>0 and deposit_amount<=total and (deposit_type<>'percentage' or deposit_value<=100)));
alter table public.invoices
  add column deposit_amount numeric(14,2) not null default 0,
  add column deposit_due_date date,
  add constraint invoices_deposit_check check (deposit_amount>=0 and deposit_amount<=greatest(total,0)
    and (source_document_id is not null or (deposit_amount=0 and deposit_due_date is null)));

-- Extend the existing private idempotency state, not the cash ledger.
alter table app_private.business_document_manual_payments
  drop constraint business_document_manual_payments_amount_check,
  add constraint business_document_manual_payments_amount_check check (amount<>0),
  ...
  add column receipt_recorded boolean not null default true;
update app_private.business_document_manual_payments set received_at=created_at;

create or replace function app_private.guard_issued_business_invoice()
...
begin
  ...
  if old.source_document_id is null and new.source_document_id is not null then
    if current_user in ('authenticated','anon') or old.amount_paid<>0 or old.stripe_amount_paid<>0
      or old.status not in ('sent','overdue') or old.total<>new.total
      or exists(select 1 from public.payment_transactions where invoice_id=old.id)
      or exists(select 1 from app_private.stripe_collection_reservations where invoice_id=old.id) then
      raise exception 'Only an unpaid booking payment without card activity can become an invoice';
    end if;
    return new;
  end if;
...
create or replace function app_private.record_business_document_receipt()
returns trigger language plpgsql security definer set search_path='' as $$
declare pending app_private.business_document_manual_payments%rowtype;
begin
  if new.source_document_id is not null and new.amount_paid is distinct from old.amount_paid then
    select * into pending from app_private.business_document_manual_payments
      where document_id=new.source_document_id and transaction_id=txid_current()
        and not receipt_recorded and amount=new.amount_paid-old.amount_paid
      order by created_at,idempotency_key limit 1 for update;
    insert into public.business_document_receipts(id,workspace_id,invoice_id,amount,received_at)
      values(coalesce(pending.receipt_id,gen_random_uuid()),new.workspace_id,new.id,new.amount_paid-old.amount_paid,coalesce(pending.received_at,now()));
    if pending.document_id is not null then
      update app_private.business_document_manual_payments set receipt_recorded=true
        where document_id=pending.document_id and idempotency_key=pending.idempotency_key;
    end if;
  end if;
  return new;
end;
...
create function public.record_document_payment_received(p_workspace_id uuid,p_document_id uuid,p_amount numeric,p_received_date date,p_idempotency_key text)
returns jsonb language sql security invoker set search_path='' as $$
  select app_private.business_document_action(p_workspace_id,'payment',p_document_id,
    p_document=>jsonb_build_object('received_date',p_received_date),p_value=>p_amount::text,p_idempotency_key=>p_idempotency_key);
$$;
create function public.record_document_refund(p_workspace_id uuid,p_document_id uuid,p_amount numeric,p_received_date date,p_idempotency_key text)
returns jsonb language sql security invoker set search_path='' as $$
  select app_private.business_document_action(p_workspace_id,'refund',p_document_id,
    p_document=>jsonb_build_object('received_date',p_received_date),p_value=>p_amount::text,p_idempotency_key=>p_idempotency_key);
$$;
revoke all on function public.record_document_payment_received(uuid,uuid,numeric,date,text),public.record_document_refund(uuid,uuid,numeric,date,text) from public,anon;
grant execute on function public.record_document_payment_received(uuid,uuid,numeric,date,text),public.record_document_refund(uuid,uuid,numeric,date,text) to authenticated;
